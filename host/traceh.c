...
static uint64_t g_log_flags = OE_LOG_FLAGS_ALL;
static bool g_initialized = false;

static uint64_t env2flags(void)
{
    uint64_t flags = OE_LOG_FLAGS_ALL;
    int fields;
...
oe_result_t oe_log_enclave_init(oe_enclave_t* enclave, bool debug_enclave)
{
    ...
    log_level_t flags = g_log_level;
    uint64_t level = g_log_flags;
    ...
    arg->debug_enclave = debug_enclave;
    arg->flags = flags;
    arg->level = level;
    // Call enclave
...
void oe_log(uint64_t flags, log_level_t level, const char* fmt, ...)
{
    if (g_initialized)
    {
        if (level < g_log_level || ((flags & g_log_flags) == 0))
            return;
    }
...
// and file operation. Console out is always on
void _log(bool is_enclave, oe_log_args_t* args)
{
    ...
    if (g_initialized)
    {
        if (args->level > g_log_level || ((args->flags & g_log_flags) == 0))
            return;
    }
